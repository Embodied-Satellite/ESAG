...
class MissionPlanner:
    ...
    def find_optimal_pass(self, satellites, target, start_time, end_time):
        """
        对多颗卫星在给定时段内寻找最优成像机会（按最早时刻优先）。
        """
        target_point = wgs84.latlon(target[0], target[1])
        candidates = []
        for sat in satellites:
            opportunity = self.analyze_satellite(sat, target_point, start_time, end_time)
            if opportunity:
    ...
    def process_tasks(self, tasks_json, satellites):
        ...
        for task_id, task in tasks_json.items():
            ...
            # 寻找最优过境
            optimal_pass = self.find_optimal_pass(
                satellites,
            ...
            if optimal_pass:  # 检查列表非空
                pass_data = optimal_pass[0]  # 取出元组 (datetime, name, tilt)
                results[task_id] = {
                    "optimal_satellite": pass_data[1],  # 卫星名
                    "pass_time": pass_data[0].strftime("%Y-%m-%d %H:%M:%S"),  # 过境时间
                    "side_look_angle": round(pass_data[2], 2)  # 侧摆角保留2位小数
                }
            else:
                results[task_id] = {
                    "optimal_satellite": None,
                    "pass_time": None,
                    "side_look_angle": None
                }
